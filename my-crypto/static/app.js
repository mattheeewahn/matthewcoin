...
// PoW — 브라우저에서 실행. Python json.dumps(sort_keys=True)와 동일하게 키 정렬
async function proofOfWork(work) {
  const target = "0".repeat(work.difficulty);
  let nonce = 0;

  while (true) {
    // Python의 sort_keys=True 와 동일한 순서: index, nonce, previous_hash, timestamp, transactions
    const blockObj = {
      index: work.index,
      nonce: nonce,
    ...
    const blockData = JSON.stringify(blockObj, Object.keys(blockObj).sort());
